...
$(document).ready(function () {
    ...
    var press = TestCafeClient.get('Automation.Press.KeyPressSimulator').press;
    ...
    asyncTest('form with one text input (type=date)', function () {
        var $form = $('<form></form>').addClass(TEST_ELEMENT_CLASS).appendTo('body'),
            $input = $('<input type="date">').addClass(TEST_ELEMENT_CLASS).appendTo($form);

        //webkit does not submit date input on enter
        checkSubmitRaisedOnEnterPress($form, $input, Util.isWebKit ? false : true);
    });
